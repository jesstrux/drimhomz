    ...
    <div class="cust-modal-content" style="position: relative;">
        ...
        @if(isset($user))
            ...
            <form id="newRental" method="POST" action="/createRental" onsubmit="addNewRental(event)">
                ...
                {{csrf_field()}}
                <label>Title</label>
                <input autocomplete="off" id="newRentalTitle" name="name" type="text" placeholder="eg. Twin room apartment" required style="font-size: 1.5em; margin-bottom: 40px;" onkeyup="setSubmit()">
                ...
                <select id="newRentalTypee" name="rental_type" required style="font-size: 1.5em; margin-bottom: 40px;" onchange="setSubmit()">
                    <option value="Apartment">Apartment</option>
                    <option value="Hostel">Hostel</option>
                    <option value="Hotel">Hotel</option>
                </select>

                <button disabled class="btn btn-primary save-new-rental hidden-xs" style="float: right; margin-righ: 8px; margin-bottom: 10px;" id="newProjectBtn" type="submit">CREATE</button>
            </form>
        @else
...
<script>
    ...
    function setSubmit(){
        ...
        var rental_type = $("#newRentalTypee").val();

        if(title.length > 0
                && price.length > 0
                && description.length > 0
                && rental_type.length > 0){
            $('.save-new-rental').removeAttr("disabled");
        }else{
